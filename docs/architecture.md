...
- `soma-bvh`: native `dataset_converter.hdf5.soma_bvh`, using shared `dataset_converter.soma` modules. Raw HDF5 SMPL root motion is converted to the SOMA Y-up frame before SOMA inversion; no post-inversion visualization-frame rotation is applied.
...
- `soma-bvh`: native `dataset_converter.nymeria.soma_bvh`, using shared `dataset_converter.soma` modules. Nymeria SMPL motion is already prepared for the SOMA Y-up path; no post-inversion visualization-frame rotation is applied.
